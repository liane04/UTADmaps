...
|---|---|:---:|:---:|:---:|---|:---:|:---:|
| **P-01** | Horário académico permanece visível após "Terminar sessão" (problema de privacidade) | ✅ A-P3 | ✅ Regra 5 | ✅ T13 (P2) | H5 / Regra 5 | **4 — Catastrófico** | B-02 |
| **P-02** | Botão "Iniciar sessão" no Perfil deixa de responder após um logout prévio (bug B-01) | ✅ A-P1 | — | ✅ T13 (P1, P2; P3 atenuado) | H3 | **3 — Maior** | B-01 |
| **P-03** | Falta de feedback informativo em acções do sistema: toggles de Definições não confirmam activação (AH); auto-centering da navegação outdoor não comunicado ao utilizador (UT) | — | ✅ Regra 3 | ✅ P1 (T7), P3 (T7) | H1 / Regra 3 | **2 — Menor** | — |
...
| **P-11** | Sem problemas detectados (a acessibilidade nas pílulas de tamanho de texto já está implementada com `accessibilityState` em [definicoes.tsx](file:///c:/Users/Utilizador/source/repos/UTADmaps/app/definicoes.tsx#L73-L80)) | — | — | — | H1 | **0 — Sem problema** | Gaps G-01 a G-03 |
...
|---|---:|---:|
| Avaliação Heurística (AH) | 9 problemas distintos | 5 também noutras técnicas |
...
> A Avaliação Heurística com três peritos identificou 9 problemas distintos (5 também noutras técnicas). Os **User Tests com 13 tarefas e 5 participantes** identificaram 11 problemas distintos (5 também noutras técnicas), acrescentando problemas qualitativos relacionados com **affordances** e **expectativas dos utilizadores** que a inspecção heurística não captou (P-04, P-14, P-15, P-16, P-17 e P-18 são exclusivos desta técnica), e — sobretudo — confirmaram empiricamente o problema de privacidade **P-01**, que as outras técnicas só tinham previsto. As Regras de Shneiderman serviram principalmente como validação cruzada; P-08 é o único exclusivo desta técnica.

---

## 4. Problemas que cruzam com a Fase 2

Três problemas desta Fase 3 cruzam directamente com bugs ou *gaps* identificados na Fase 2, reforçando que a avaliação manual de usabilidade detecta problemas que as ferramentas automáticas não captam, e mostrando que tais problemas são **reais e persistentes**.
...
A combinação das três técnicas de avaliação de usabilidade identificou **17 problemas distintos** no UTAD Maps, dos quais **13 são prioritários para correcção** (severidade ≥ 2). O problema mais grave é **P-01** (catastrófico, privacidade) seguido de **P-02** (maior, gestão de sessão).

A convergência entre técnicas é elevada: **35 % dos problemas (6 em 17) foram detectados por mais do que uma técnica**, e **apenas um deles — P-01 (privacidade) — foi detectado pelas três em simultâneo (sendo P-03 validado por duas técnicas, SH + UT)**. Esta convergência valida metodologicamente a combinação adoptada e indica que estes seis problemas devem ser prioritários para qualquer iteração subsequente do produto.
...
O cruzamento com a Fase 2 mostra que **três problemas** documentados como bugs ou *gaps* de acessibilidade foram **revalidados** pelas técnicas de usabilidade. Em particular, os **bugs B-01 e B-02** — ambos ligados ao ciclo de logout/login — são agora confirmados empiricamente e devem ser corrigidos antes de qualquer entrega futura.
