...
| C | Testes com Utilizadores       | 5 participantes (equipa, P1–P5)      | 5 sessões × 13 tarefas com tabela, *checklist* e questionário Likert pós-teste    |
...
Dois problemas foram identificados pelas três técnicas em simultâneo: **P-01 (privacidade do horário)** e **P-03 (feedback insuficiente)**. O caso de **P-01** ganhou peso decisivo nesta revisão: o cenário, antes apenas previsto pela inspecção heurística e por Shneiderman, foi **directamente observado nos User Tests** em T13 pelo Participante 2, em que após o *logout* o horário permaneceu exposto sem qualquer sessão activa. São, portanto, os **problemas mais robustamente fundamentados** desta avaliação.

A distribuição global por severidade é: 1 problema catastrófico (P-01), 1 problema maior (P-02), 11 problemas menores e 5 problemas cosméticos. Os testes com utilizadores, ao serem organizados em 13 tarefas que reproduzem um fluxo natural completo, revelaram **seis problemas exclusivos desta técnica** — P-04 (tempo elevado e erros em T2), P-14 (*affordance* do cartão de aula no Horário), P-15 (chave do Inforestudante), P-16 (*affordance* do card "Próxima Aula" no Perfil), P-17 (FAQ sem campo de pesquisa) e P-18 (acesso a Definições não imediatamente óbvio) — que apenas a observação directa de utilizadores em fluxos extensos permitiu detectar.
...
Com base nos 18 problemas identificados, propõe-se um plano de **19 melhorias** organizadas em três *sprints* sequenciais e classificadas numa matriz de **Impacto × Esforço**. O **Quadrante I** (Impacto Alto × Esforço Baixo) reúne **seis melhorias** consideradas de prioridade máxima e estimadamente concluíveis em **um dia de trabalho**, resolvendo o problema catastrófico, o problema maior e os problemas menores com maior convergência entre técnicas. A Tabela 21 resume estas seis melhorias.
...
| M-04 | *Toast* de confirmação quando o utilizador activa Alto Contraste, muda tema, altera tamanho de texto ou muda idioma | P-03 menor | Pequeno  |
| M-05 | `accessibilityLiveRegion="polite"` na barra de navegação outdoor (mudanças de manobra + *auto-centering*) | P-03, P-09 menores | Pequeno  |
| M-06 | `ActivityIndicator` durante a chamada de pesquisa ao *backend*                                  | P-06 menor         | Trivial  |

Os restantes 13 problemas são abordados num Sprint 2 (melhorias de fluxo, 2 dias — notificação de chegada ao destino, atalho "Próxima aula" no Mapa, modal de importação melhorado com captura de ecrã do Inforestudante) e num Sprint 3 (polimento, 1 dia — mensagens de erro, legenda do mapa, glossário no *onboarding*, *tooltips* de acessibilidade, raio dos cantos, ícones de *affordance* nos *cards*, *badge* 2D/3D, *chevron* no card Próxima Aula, campo de pesquisa nas FAQs e destaque da entrada Definições no Perfil), detalhados em `MELHORIAS_PROPOSTAS.md`.
